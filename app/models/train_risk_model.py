...
from app.utils.preprocessing import load_data
...
def main():
    ...
    if "yield_kg_per_ha" not in df.columns:
        print("❌ 'yield_kg_per_ha' column not found.")
        return

    if "crop_extinction_risk" not in df.columns:
        print("❌ 'crop_extinction_risk' column not found.")
        return
    ...
    X = df.drop(columns=["yield_kg_per_ha", "crop_extinction_risk"])
